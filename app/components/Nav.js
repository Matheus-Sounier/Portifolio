    ...
    <>
      <nav ref={navRef} className="flex items-center gap-4 text-sm font-mono">
        {LINKS.map((link) => {
          ...
          if (link.external) {
            return (
              ...
              >
                <span className="text-red-400">[{link.key}]</span> {link.label}
              </a>
            )
          }
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={active ? 'text-zinc-100' : 'text-zinc-400 hover:text-zinc-100'}
            >
              <span className="text-red-400">[{link.key}]</span> {link.label}
            </Link>
          )
        ...
        <button
          ...
          className="ml-auto text-rose-500 hover:text-rose-400 transition-colors duration-150"
          aria-label="search"
        >
          /
        </button>
      ...
      {open && (
        <div
          className="fixed left-0 right-0 bottom-0 bg-zinc-950 overflow-y-auto"
          style={{ top: navHeight }}
        >
          <div className="max-w-2xl mx-auto px-6 pt-10">
            <div className="flex items-center gap-2 border border-rose-900 rounded-lg px-4 py-3 focus-within:border-rose-500 transition-colors duration-150">
            ...
            <div className="mt-6 divide-y divide-zinc-900">
              ...
              {results.map((result) => (
                <button
                  key={result.id}
                  onClick={() => goTo(result)}
                  className="w-full flex items-center justify-between gap-4 py-3 text-left text-zinc-200 hover:text-rose-400 transition-colors duration-150"
                >
